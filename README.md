...
**3. Choosing the query-parsing approach.** For Step 0 of the loop, the AI initially proposed parsing the query with **regex / string-splitting** to pull out the description, size, and price. I revised this to an **LLM-based parser** (`_parse_user_query`, Groq at temperature 0.0 with few-shot examples in `prompts.py`). The reason was that regex handled the price reasonably but was brittle on size — it couldn't reliably normalize natural phrasing like `"medium"`/`"size M"`/`"a medium"` into the canonical `"M"` the dataset uses, and broke on unusual word order. The LLM parser normalizes sizes and ignores filler words, and I kept a regex-free fallback (use the raw query as `description`, `size`/`max_price` = `None`) so a parser failure still degrades gracefully instead of crashing.
